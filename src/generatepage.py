...
def generate_page(from_path, template_path, dest_path, base_path):
    ...
    x = template_file.replace("{{ Title }}", title).replace("{{ Content }}", html_string).replace('href="/', f'href="{base_path}').replace('src="/', f'src="{base_path}')
...
def generate_page_recursive(dir_path_content, template_path, dest_dir_path, base_path):
    ...
    for file in files:
        ...
        if os.path.isfile(dir_path):
            ...
            generate_page(dir_path, template_path, dest_path, base_path)
        else:
            dest_path = os.path.join(dest_dir_path, file)
            generate_page_recursive(dir_path, template_path, dest_path, base_path)
